...
export default function CookieConsent() {
  ...
  useEffect(() => {
    const consent = localStorage.getItem('coltura-cookie-consent');
    if (!consent) {
      setVisible(true);
    }
  }, []);
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-[100] border-t border-line/50 bg-card/95 backdrop-blur-md">
      <div className="mx-auto flex max-w-6xl flex-col items-start gap-4 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-8">
        ...
        <div className="flex shrink-0 gap-3">
          <button
            ...
            className="rounded-lg border border-line px-5 py-2.5 text-sm font-medium text-muted transition-colors hover:border-brand/30 hover:text-ink"
          >
            Decline
          </button>
          <button
